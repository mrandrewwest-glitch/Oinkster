...
export class MockProvider {
  ...
  seedTransactions(providerUserId, txs) {
    ...
    list.push(...txs.map((t) => ({ id: t.id ?? randomUUID(), postDate: t.postDate ?? new Date().toISOString(), ...t })));
  ...
  async createDebit({ providerUserId, amountCents, description, reference }) {
    const debit = { id: `mock-debit-${randomUUID()}`, providerUserId, amountCents, description, reference, status: 'settled' };
  ...
  async createPayout({ amountCents, to, description, reference }) {
    const payout = { id: `mock-payout-${randomUUID()}`, amountCents, to, description, reference, status: 'settled' };
